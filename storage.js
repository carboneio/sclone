...
function syncFiles (filesToUploadTarget, filesToDeleteTarget, filesToUploadSource, filesToDeleteSource, mode, callback) {
  try {
    ...
    execQueue(`delete-files-target`, filesToDeleteChunks, function(objects, next) {
    ...
    }, { concurrency: 10, delay: 0, retry: 0, logQueueStatus: logQueueStatus }, function(err) {
      if (err) {
      ...
      return execQueue('upload-from-source-to-target', filesToUploadTarget, function(object, next) {
        ...
        { concurrency: 15, delay: 0, retry: 0, logQueueStatus: logQueueStatus },
        function(err) {
          if (err) {
          ...
          function(err) {
            ...
            { concurrency: 10, delay: 0, retry: 0, logQueueStatus: logQueueStatus },
            function(err) {
              if (err) {
